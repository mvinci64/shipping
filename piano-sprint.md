...
- **Corretto in corsa**: il "caso limite `fetch_ultimo_lotto`" (rischio riportato sotto) non è solo un dettaglio di query — `easyfatt.tmovmagazz` è scritta dall'**ETL EasyFatt → BI (`bi.viscotta.com`), lanciato a mano, senza orario fisso**. Nessuna etichetta con lotto reale è affidabile prima che l'ETL sia stato rilanciato dopo il carico del giorno. Sequenza completa e opzioni di mitigazione in `procedura-giorno-produzione.md`. Da fare in questo sprint: opzione 3 lì descritta — bloccare esplicitamente `GET /cartonizzazioni/{order_number}/etichette-colli` se l'ultimo carico trovato non è del giorno atteso, invece di stampare in silenzio un lotto vecchio
...
| **Nuovo**: `easyfatt.tmovmagazz` è scritta dall'ETL EasyFatt→BI, lanciato a mano senza orario fisso — l'etichetta lotto non è affidabile se richiesta prima che l'ETL sia stato rilanciato dopo il carico del giorno (vedi `procedura-giorno-produzione.md`) | Rischio di lotto vecchio o mancante in etichetta, silenzioso | Sprint 3 |
